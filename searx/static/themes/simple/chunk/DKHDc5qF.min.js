...
</svg>`,u=e=>{if(e.querySelector(`.btn-toggle-content`))return;let t=e.innerHTML;if((e.textContent||``).length<=c)return;let n=0,r=-1,i=!1;for(let e=0;e<t.length;e++){let a=t[e];if(a===`<`)i=!0;else if(a===`>`)i=!1;else if(!i&&(n++,n>=c&&r===-1)){let n=t.indexOf(` `,e);r=n!==-1&&n-e<20?n:e}}r!==-1&&r<t.length-20&&(e.innerHTML=`${t.substring(0,r)}<span class="content-hidden">${t.substring(r)}</span><button class="btn-toggle-content" aria-expanded="false">${l}</button>`)},d=()=>{let e=document.querySelectorAll(`.result .content`);for(let t of e)u(t)},f=()=>{d()};document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,f):f(),t(`click`,`.btn-toggle-content`,function(){let e=this.parentElement;if(!e)return;let t=e.querySelector(`.content-hidden`);t&&(this.getAttribute(`aria-expanded`)===`true`?(t.style.display=`none`,this.setAttribute(`aria-expanded`,`false`)):(t.style.display=`inline`,this.setAttribute(`aria-expanded`,`true`)))}),window.addEventListener(`scroll`,()=>{let e=document.getElementById(`backToTop`),t=document.getElementById(`results`);if(e&&t){let e=(document.documentElement.scrollTop||document.body.scrollTop)>=100;t.classList.toggle(`scrolling`,e)}},!0);
//# sourceMappingURL=DKHDc5qF.min.js.map